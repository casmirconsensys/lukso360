...
const FeedCard = ({ feed }) => {
  ...
  return isPortrait ? (
    <div
      style={{ color: "white" }}
      className={`${styles.cardContainer} ${fonts.mont}`}
    >
      <div className={styles.metaContainer}>
        {feed &&
          (feed.fileType.startsWith("audio") ? (
            <img
              className={`${styles.playlistArt}`}
              src={feed.albumArt?._url}
              onClick={() => dispatch(setStreamUrl(feed.image))}
            />
        ...
        <div className={styles.metaText}>
          <div className={styles.topRowContainer}>
            ...
            <Button
              ...
              onClick={() => router.push(feed.metapath)}
            />
          </div>
          <h3 className={styles.playlistTitle}>
            {feed.title}{" "}
            {feed.artist && (
              <span className={styles.artist}>{feed.artist}</span>
            )}
          </h3>
      ...
      <div className={styles.bottomRowContainer}>
        <Button type="secondary" isOutline={1} text="Subscribe" />
        <Button
          type="secondary"
          isOutline={1}
          text="Subscribe With Crypto"
          onClick={() => subscribeWithWallet(feed.id)}
        />
      ...
      {feed.feedType === "Playlist" && (
        <>
          {feed.playlist
            .slice(0, showAll ? feed.playlist.length : 3)
            .map((item) => (
              <div key={item.id}>
          ...
          {!showAll && (
            <button
              ...
              className="btn btn-secondary btn-block playlist-view-more"
            >
              View {feed.playlist.length} tokens
            </button>
          )}
    ...
    >
      <div className={styles.metaContainer}>
        ...
        <div className={styles.metaText}>
          ...
          <h4 className={styles.username}>{feed.username}</h4>
          <h5 className={styles.username} style={{ marginBottom: "28px" }}>
            {feed.description}
          </h5>
          <div className={styles.bottomRowContainer}>
            {/* <Button type='secondary' isOutline={1} text='Subscribe'/> */}
        ...
        <>
          ...
          {!showAll && (
            <button
              type="button"
              onClick={() => setShowAll(true)}
              className="btn btn-secondary btn-lg btn-block playlist-view-more"
            >
              View {feed.playlist.length} tokens
            </button>
          )}
