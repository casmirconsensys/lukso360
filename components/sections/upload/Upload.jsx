...
import {
  ...
  setTokenArtist,
  setTokenAlbumArt,
} from "../../../features/redux/upload/upload-slice";
...
//   const metadataResult = await fetchAgent.fetchIPFSMetadata(
...
//     if (file.type.startsWith("audio")) setMusicSelected(true);
//     else if (file.type.startsWith("glb")) setModelSelected(true);
//   }
...
//       throw error; // Rethrow the error or handle as necessary
//     }
//   };
//   const contract = new web3.eth.Contract(
//     tokenContractABI,
//     TOKEN_CONTRACT_ADDRESS.current
//   );
//   const receipt = async => await contract.methods
//     .createItem(metadataUrl)
//     .send({ from: ethereum.selectedAddress });
//   setUploadStatus("Minting");
//   setMintReceipt(receipt);
...
//       .approve(MARKETPLACE_CONTRACT_ADDRESS.current, tokenId)
...
//       alert(
//         `NFT name contains special characters.\nPlease remove any special characters ${format}.`
//       );
//       return;
//     }
//     if (format.test(tokenDesc)) {
//       alert(
//         `NFT description contains special characters.\nPlease remove any special characters ${format}.`
//       );
//       return;
//     }
...
//       nftAlbumArt = new ipfs.File(
//         `nftAlbumArt_${Math.floor(Math.random() * 9999999) + 1}`,
//         tokenAlbumArt
//       );
...
//       metadata: tokenUri,
//     };
//     const nftFileMeta = new ipfs.File("metadata.json", {
...
//       marketPlaceABI,
//       MARKETPLACE_CONTRACT_ADDRESS.current
//     );
...
//     MARKETPLACE_CONTRACT_ADDRESS.current
//   );
//   return contract.methods.addItemToMarket(
//     contractAddress,
//     tokenId,
//     ipfs.Units.ETH(price)
//   );
// };
...
// };
// {
//   //added this to close the bottom return statement function closing brackets
                    ...
                    {fileSelected &&
                      (fileName.name.length >= 10
                        ? `${fileName.name.substr(
                            ...
                            fileName.name.length - 5
                          )}`
                        : fileName.name)}
                  </small>
                  {fileSelected && (
                    <Button text="Change" type="small" onClick={ofd} />
                  )}
                  {(musicSelected || modelSelected) && (
                    <small className="color-primary">|</small>
                  )}
                  {musicSelected && (
                    <Button
                      text="Add Album Art"
